import { clamp, degreeToRadians, detectTrackpad, getUntransformedRect, isMultipleOf } from '../core/helpers';

import { Zoompinch } from '../core/zoompinch';
...
export class ZoomPinchElement extends HTMLElement {
  ...
  private connectedCallback() {
    ...
    this.engine = new Zoompinch(
      this.contentEl,
      { top: !isNaN(initialOffsetTop) ? initialOffsetTop : 100, left: !isNaN(initialOffsetLeft) ? initialOffsetLeft : 0, right: !isNaN(initialOffsetRight) ? initialOffsetRight : 0, bottom: !isNaN(initialOffsetBottom) ? initialOffsetBottom : 0 },
      initialTranslateX,
      initialTranslateY,
      initialScale,
      initialRotate,
  ...
  private attributeChangedCallback(name: string, _: string, value: string) {
    ...
    switch (name) {
      ...
      case 'offset-left':
        ...
        this.engine.offset = { top: offsetTop, right: offsetRight, bottom: offsetBottom, left: offsetLeft };
        this.engine.update();
        break;
    }
  }
